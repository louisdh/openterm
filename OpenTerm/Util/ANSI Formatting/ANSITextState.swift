	...
	mutating func parse(escapeCodes: String) {
		...
		while index < codes.endIndex {
			...
			if code == 0 { reset() }
				...
				// Foreground color
			else if let foregroundColor = ANSIForegroundColor(rawValue: code) { self.foregroundColor = foregroundColor.color } else if code == ANSIForegroundColor.custom { let result = customColor(codes: Array(codes.suffix(from: index + 1))); readCount += result.readCount; foregroundColor = result.color }

				// Background color
			else if let backgroundColor = ANSIBackgroundColor(rawValue: code) { self.backgroundColor = backgroundColor.color } else if code == ANSIBackgroundColor.custom { let result = customColor(codes: Array(codes.suffix(from: index + 1))); readCount += result.readCount; backgroundColor = result.color } else if let fontState = ANSIFontState(rawValue: code) {
				switch fontState {
